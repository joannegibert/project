...
This project is designed to run in a standard Unix-like environment (Linux/macOS) or Windows.
...
This command:
1. Calls the compiled C executable (`bin/seird_model`) from within the MATLAB script.
2. Generates the scenario output files (`results/scenario_*.csv`).
3. Computes the summary table (`results/summary_results.csv`).
4. Saves the five figures into the `results/` directory.
