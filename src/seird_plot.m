...
for i = 1:num_files
    ...
    fname = files(i).name;
    csv = readtable(fullfile(files(i).folder,fname));   % read scenario i CSV into a table
...
figure(2); clf;
csv4 = readtable(fullfile(files(i).folder,files(4).name)); % consider scenario_4.csv only
...
for i = 1:5
    saveas(figure(i), fullfile("../results", ['figure_' num2str(i) '.png']));
